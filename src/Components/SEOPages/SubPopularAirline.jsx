...
import SEOAirlinesData from '../../Constant/SEOAirlinesData.js';

const SubPopularAirline = () => {
  ...
  const DepartCode = searchParams.get('Departure');
  const ArrivalCode = searchParams.get('Arrival');
  console.log("DepartCode",DepartCode);
  console.log("ArrivalCode",ArrivalCode);
  ...
  const {airlineName ,cityName } = useParams();

//   console.log("destination 11:",cityName);
//   console.log("airlien name 22:",airlineName);
  ...
  useEffect(() => {
    ...
    // );
    const departCity = `${DepartCode}`
    const arrivalCity = `${ArrivalCode}`;
...
export default SubPopularAirline;