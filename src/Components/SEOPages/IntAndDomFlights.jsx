...
// ---------------date----------------------
  ...
  useEffect(() => {
    ...
    let arrivalCity
      ...
      console.log("parts----v1",parts);
  
       departCity = parts[0].toLowerCase();
  ...
  const fetchData = async () => {
    try {
      ...
      if(tripType === "OneWay" || tripType === "Round")
      {
        ...
        console.log("alternateRates--v1",alternateRates);

        const weekDates = alternateRates.flatMap(itm=>itm.date);
        setWeekDates(weekDates)
        const weekDatePrice = alternateRates.map(item =>item.price[0]);
        setWeeklyFare(weekDatePrice);
        // const fareDates = [...weekDates ,weekDatePrice]
        // console.log("weekDatePrice",fareDates);
        // console.log("datesData",weekDates);
        setAlerRates(alternateRates);
      }

    
      if ( fetchedFlightData.length > 10) {
  ...
  const getCityName = (cityCode) =>{
    const cityName = airportDetails?.find(items =>items.iata_code === cityCode);
  ...
  useEffect(() => {
    ...
    if(!FooterFlights){
      const  matchedItem = CityBackgroundImages.find((item)=>item.pathName ===from);
      setBackgroundBanner(matchedItem);
    }else{
      const  matchedItem = InternationRoutes.find((item)=>item.pathName ===from);
      setBackgroundBanner(matchedItem);
    }
          ...
          <Fragment> 
              {/* {
                !isMobile || backgroundBanner?.backGroundImg ? (
                  <div className='backgroundWithBanner' style = {{backgroundImage :`url(${backgroundBanner?.backGroundImg})`}}>
              ...
              } */}
              {
                !(isMobile || !backgroundBanner?.backGroundImg) && (
                  <div 
                    ...
                    style={{ backgroundImage: `url(${backgroundBanner.backGroundImg})` }}
                  >
                    <h3 className='CityBanner_heading'>
                      <Typewriter
                        words={[backgroundBanner?.Heading || '']}
              ...
              <Fragment>
                <div className="container">
                  <ItemsToShowProvider totalResults={totalResults} apiData={getCombinedData} searchDataArr={searchDataArr} filterDataArr={filterDataArr} >
                    ...
                    <div className={FooterFlights === false ? 'row  banner_adjustment m-0' :'row m-0'}>
                      ...
                      <div className="col-md-9 mt-2 pr-0 mypadding">
                        ...
                        {FooterFlights ?
                            DomesticFlightsJason.find((flightData) => flightData.flightName === from)
