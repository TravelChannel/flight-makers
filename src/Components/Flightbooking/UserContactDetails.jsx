...
const UserContactDetails = (props) => {
  ...
  const returnDate =
    flightDetails.groupDescription[1]?.departureDate || "01-01-1900";
    ...
    if (userVerName && isLogin) {
      ...
      if (value !== `+${dialCode} ${phoneNumber}`) {
        if (value.includes(dialCode)) {
          const numericPhoneNumber = value
            .replace(dialCode, "")
            .replace(/\D/g, "");
          setPhoneNumber(numericPhoneNumber);
        } else {
  ...
  const handleNavigation = async () => {
    try {
      ...
      localStorage.setItem(
        "userDetails",
        JSON.stringify(finalObject.pnrBookings[0])
      );
  ...
  useEffect(() => {
    const newArrivalLocations = flightDetails.groupDescription.map(
      (item) => item.arrivalLocation
    );
    const OperatingAirlines = flightDetails.schedualDetGet.flatMap((item) =>
      item.map((itm) => itm.carrier.operating)
    );
    const CommissionData = {
      Destinations: newArrivalLocations,
      OperatingAirline: OperatingAirlines,
                  ...
                  <div className="d-flex justify-content-start wrap  flex-wrap">
                    ...
                    {isLogin ? (
                      ...
                      <div>
                        <button
                          type="button"
                          onClick={sendOTPHandller}
                          className="btn btn-primary iti_otp_button"
                        >
                          Get OTP
                        </button>
                      </div>
                  ...
                  <div className="traveler_detail_main">
                    <div className="trav_data_main">
                      ...
                      <div className="td_adults_details">
                        {Array.from({ length: passengerDetails }).map(
                          (_, index) => (
                            <div key={index} className="each_passanger_record">
                              ...
                              <div className="adults_input_fields  ">
                                <div className="important_note">
                                  <span className="adult_pre_icon align-self-center ">
                                    <TipsAndUpdatesIcon />
                                  </span>
                                  {isSmallScreen ? (
                                ...
                                <div className="row p-0 m-0">
                                  ...
                                  {index < adults &&
                                  flightDetails?.schedualDetGet?.[0]?.[0]
                                    ?.carrier?.operating === "PF" ? (
                                    <div className="col-md-4 mb-3">
                    ...
                    <div className="move_payment_button">
                      <button
                        ...
                        className={`btn btn-primary continue_button  ${
                          travelFormValid() ? "c-pointer" : "c-not-allowed-btn"
                        }`}
                        disabled={!travelFormValid()}
